...
class HudDamageBar {
	...
	protected updateLabel() {
		...
		this.label.label = intl.t("hud.hull", {
...
class HudCannon {
	...
	protected updateHasAmmo() {
		...
		this.alert(intl.t("hud.cannon.noAmmo"));
	}

	protected updateHasCrew() {
		if (this.cannon.alreadyManned()) return;
		this.alert(intl.t("hud.cannon.noCrew"));
	}
...
class HudEngine {
	...
	protected updateHasFuel() {
		...
		this.alert(intl.t("hud.engine.noFuel"));
	}

	protected updateHasCrew() {
		if (this.engine.alreadyManned()) return;
		this.alert(intl.t("hud.engine.noCrew"));
	}
...
class HudCounters {
	...
	constructor(args: HudCountersArgs) {
		...
		this.addRow(
			intl.t("hud.info.day", { day: this.player.game.difficulty + 1 }),
		);
		this.addRow(intl.t("hud.info.dayTime"), (label) => {
		...
		addStat(intl.t("hud.info.finance.dayProfit"), (player) => {
		...
		addStat(intl.t("hud.info.finance.expenditures"), () => 0);

		this.addRow("---");
		this.addRow(intl.t("hud.info.velocity"), (label, player) => {
			if (player.possessed == null) return;
			label.label = `${(player.possessed.vel.length() / 10).toFixed(2)} m/s`;
		});
		this.addRow(intl.t("hud.info.thrust"), (label, player) => {
		...
		this.addRow(intl.t("hud.info.kills"), (label, player) => {
...
class Hud extends CanvasPanel {
	...
	tick(deltaTime: number) {
		...
		this.leaveIslandLabel.label = this.player.possessed.inDanger()
			? intl.t("hud.status.leaveChase")
			: intl.t("hud.status.leave");
...
class HudRenderer {
	...
	protected renderPauseScreen() {
		...
		ctx.fillText(intl.t("hud.paused"), game.width / 2, game.height / 4);
		ctx.font = "18px sans-serif";
		ctx.textBaseline = "top";
		ctx.fillText(
			intl.t("hud.pauseMessage"),
			game.width / 2,
			game.height / 4 + 55,
		);
	...
	protected renderDeathScreen(): boolean {
		...
		ctx.textBaseline = "middle";
		ctx.textAlign = "center";
		ctx.fillText(intl.t("hud.status.rip"), game.width / 2, game.height / 2);
		ctx.font = "18px sans-serif";
		ctx.textBaseline = "top";
		ctx.fillText(
			intl.t("hud.status.tryAgain"),
			game.width / 2,
			game.height / 4 + 55,
		);
