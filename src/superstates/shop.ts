...
import { computeResellCost, FoodItem, FuelItem } from "../inventory";
...
import { arrayCounter, costString, moneyString, weightString } from "../util";
		...
		this.damageLabel.label =
			this.part.damage <= 0
				...
				: intl.t("intermission.drydock.part.damaged", {
						percent: `${((100 * this.part.damage) / this.part.maxDamage).toFixed(0)}%`,
...
class DrydockInventoryItemWidget extends Pane<DrydockInventoryItemWidgetArgs> {
	...
	protected buildPane(args: DrydockInventoryItemWidgetArgs & CanvasPanelArgs) {
		...
		if (this.item instanceof ShipPart) {
			...
			}).label(intl.t("intermission.drydock.item.action.install"), labelArgs);
		}
	}
...
>(
	...
	for (const widget of widgets) {
		...
		if (idx === -1 || shouldSkip?.(remaining[idx])) {
			widget.destroy();
		} else {
...
> {
	...
	protected buildPane(args: ShopItemWidgetArgs & CanvasPanelArgs) {
		...
		new CanvasButton({
			...
			intl.t(`intermission.shop.${this.item.type === "crew" ? "hire" : "buy"}`),
			{
				font: "bold $Hpx sans-serif",
				color: "#fff",
				fillY: 1.0,
				...
				stat: function (this: StatRow) {
					...
					if (this.player.money < totalSalary) {
						...
						return (
							...
							(hasUnhappy
								? ""
								: intl.t("intermission.drydock.stats.salary.info.strikeSoon", {
