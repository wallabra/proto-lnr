			...
			...(makeup != null && this.manned && !this.alreadyManned()
				? [
						typeof this.manned === "number"
							...
							: intl.t("shopinfo.manning.needsAny"),
					]
				: []),
			...(this.canUninstall ? [] : [intl.t("shopinfo.cannotUninstall")]),
		];
	}
...
export class Crew implements ShipItem {
	...
	shopInfo(makeup?: ShipMakeup): string[] {
		return [
			...
			intl.t("shopinfo.crew.foodIntake", {
				foodIntake: this.caloricIntake.toFixed(0),
			}),
			intl.t("shopinfo.crew.strength", {
			...
			...(makeup == null
				...
				: [
						this.manningPart != null
							? intl.t("shopinfo.crew.manning", {
									manningType: this.manningPart.getItemLabel(),
								})
							: makeup.captain === this
								? intl.t("shopinfo.crew.captain")
								: intl.t("shopinfo.crew.idle"),
					]),
		];
	}
...
export class Armor extends ShipPart implements Required<ArmorArgs> {
	...
	override shopInfo(): string[] {
		...
		return [
			intl.t("shopinfo.armor.absorption", info),
			intl.t("shopinfo.armor.deflection", info),
			intl.t("shopinfo.armor.wear", info),
			intl.t("shopinfo.armor.overwhelm", info),
		];
	}
}
	...
	override shopInfo(): string[] {
		return [
			...
			...(this.fuelType == null
				? []
				: [
						intl.t("shopinfo.engine.fuelCost", {
							fuelCost: (this.fuelCost * 60).toFixed(2),
						}),
					]),
			intl.t("shopinfo.engine.thrust", {
...
function generateShipName() {
	const namegen = intl.t("ship.namegen", { returnObjects: true }) as {
		adjectives: string[];
		titles: string[];
		cores: string[];
		suffixes: string[];
