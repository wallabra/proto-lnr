...
export class Ship {
  ...
  dragMixin() {
    ...
    this.phys.waterDrag = function() {
      const alpha = 1 - Math.abs(Vec2(1, 0).rotateBy(this.angle).dot(this.vel.norm()));
  ...
  steer(deltaTime, angleTarg) {
    ...
    const steerCompensate = angDiff(umod(this.angle + this.phys.angVel, Math.PI * 2), angleTarg);
